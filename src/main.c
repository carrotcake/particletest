...
static double repulsion_radius = 1.75, repulsion_factor = 0.25, brown_factor = .25;
static bool b_emtrrplsn = true;
...
static const size_t CELL_SIZE = PARTICLE_SIZE * 2;
...
inline static void RepulseBox(Box *p, const Box *repulsor, const double dt) {
...
void DoBoxRepulsion(const double dt) {
    Box *boxes = e.particles;
...
void HandleInput(Emitter *e) {
    ...
    if (frames % 5 == 0) {
        ...
        if (IsKeyDown(KEY_LEFT_BRACKET) && repulsion_radius > .01) {
            repulsion_radius -= .01;
        }
        if (IsKeyDown(KEY_RIGHT_BRACKET) && repulsion_radius < 10) {
            repulsion_radius += .01;
        }
        if (IsKeyDown(KEY_SEMICOLON) && repulsion_factor > -10.) {
            repulsion_factor -= .01;
        }
        if (IsKeyDown(KEY_APOSTROPHE) && repulsion_factor < 10.) {
            repulsion_factor += .01;
        }
...
void DoTextStuff(Emitter *e) {
    ...
    sprintf(fpsbuf, "FPS: %d\n%lu particles", GetFPS(), e->count);
    ...
    if (b_menuopen) {
        ...
        sprintf(flagsbuf,
                ...
                "%.2f)\n\nNewton's 3rd "
                "[N]: %s\n\n"
                "Repulsion [P]: %s (Emitter Repulsion [E]: %s)\n\t(< [ ] >radius = %.2f,\n\t < ; ' "
                ">factor = %.2f)\n\nXP "
                "Solitaire[L]: %s\n\n"
                "Emitter Size: %.0fpx\n\t(< , . >)\n\n"
